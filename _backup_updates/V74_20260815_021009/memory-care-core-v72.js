...
window.MemoryCare = window.MemoryCare || (()=>{
  ...
  const write=async(key,value)=>{const cfg=await ready();return await cfg.write(key,value)};
  ...
  return {BUILD,esc,iso,parse,today,daysBetween,addDays,range,clamp,normalizeName,uuid,read,write,toast,status,fmtDate,fmtShort,language};
})();
