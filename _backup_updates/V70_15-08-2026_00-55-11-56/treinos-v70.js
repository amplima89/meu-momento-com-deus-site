...
(() => {
  ...
  function renderStrength(root,workout,session) {
    ...
    const buildCard=(ex,idx,pair=null)=>{
      ...
      return `
        <article class="exercise-card ${done?"done":""} ${missed?"missed":""} ${isCurrent?"current":""} ${isOpen?"open":""} ${pair?`conjugated-exercise conjugated-exercise--${pair.position===1?"a":"b"}`:""}" data-exercise="${esc(ex.exercicioId)}">
    ...
    const finishArea=session.status==="em_andamento"
      ? `<article class="card finish-workout-card">
          <div><span class="treino-kicker">${p.pct===100?"CONCLUINDO AUTOMATICAMENTE":"TREINO ABERTO"}</span><h2>${p.done}/${p.total} exercícios</h2><p>${p.pct===100?"Tudo foi concluído. O Memory está encerrando o treino automaticamente.":"O treino permanece aberto até o fim do dia. Você pode sair e voltar depois para concluir o que ficou pendente."}</p></div>
          ${p.pct===100?'<span class="finish-auto-badge">✓ 100%</span>':'<span class="finish-auto-badge">Em andamento</span>'}
        </article>`
  ...
  function toggleSeries(exerciseId,seriesNo) {
    ...
    if(concluindo && progress(session).pct===100){
      closeRestTimer();
      window.setTimeout(()=>maybeAutoFinishWorkout(session),80);
      return;
    }
  ...
  function isFlexibleMonthlyMeta(meta){
  ...
  function metaActiveByDate(meta,date){
    ...
    if(isFlexibleWeeklyMeta(meta)||isFlexibleMonthlyMeta(meta)) return true;
  ...
  function monthlyCompletionCount(meta,date){
    if(!isFlexibleMonthlyMeta(meta)||!state.atividadesData) return 0;
    const base=new Date(`${date}T12:00:00`);
    const cursor=new Date(base.getFullYear(),base.getMonth(),1,12);
  ...
  function flexibleTarget(meta){
    if(isFlexibleMonthlyMeta(meta)) return Math.max(1,Number(meta?.metaMensal||meta?.quantidade||1));
  ...
  function linkedActivityMetasForWorkout(session){
    // V67: vínculo simples e literal.
    // Se houve musculação, todas as metas marcadas para integração E programadas para aquele dia são concluídas.
    if(!isMusculacaoSession(session)) return [];
    return activeActivityMetas(session.data).filter(meta=>{
      if(meta?.associadaTreinoFisico!==true) return false;
      if(isFlexibleWeeklyMeta(meta)) return weeklyCompletionCount(meta,session.data)<flexibleTarget(meta);
      if(isFlexibleMonthlyMeta(meta)) return monthlyCompletionCount(meta,session.data)<flexibleTarget(meta);
      return true;
    });
  }

  function primaryActivityMetasForWorkout(date=todayIso()){
    // O abono que bloqueia o treino continua procurando uma meta com cara de "Treino/Musculação".
    // Metas auxiliares vinculadas (ex.: Cardio) não devem bloquear a musculação sozinhas.
    ...
    // Compatibilidade com a configuração antiga somente enquanto não houver uma meta principal vinculada.
  ...
  async function markWorkoutActivity(session) {
    ...
    if(!isMusculacaoSession(session)) return {ok:false,reason:"nao-musculacao",metas:[],newlyMarked:[]};
    ...
    for(const meta of metas){
      ...
      const isFlexible=isFlexibleWeeklyMeta(meta)||isFlexibleMonthlyMeta(meta);
      const nextValue=isFlexible?Math.max(1,Number(previous?.valor||0)):1;
      const origem=isFlexibleWeeklyMeta(meta)?"treino_semanal_flexivel":isFlexibleMonthlyMeta(meta)?"treino_mensal_flexivel":"treino";
      window.MMCD.setRegistro(state.atividadesData,session.data,meta.id,{
        concluida:true,abonada:false,valor:nextValue,texto:"",observacao:previous?.observacao||"",origem
      });
  ...
  function activityIntegrationHtml() {
    ...
    const rows=linked.length?linked.map(meta=>{
      return `<div class="integration-summary-row"><span>🏋️</span><div><strong>${esc(meta.nome)}</strong><small>Será concluída quando houver musculação e esta meta estiver programada para o dia.</small></div></div>`;
    }).join(""):`<div class="empty">Nenhuma meta vinculada à musculação ainda.</div>`;
    return `<article class="card settings-block">
      <div class="section-head"><div><p class="eyebrow">Integração automática</p><h2>Musculação → Atividades</h2><p class="muted">É simples: você escolhe quais metas estão vinculadas. Quando fizer musculação, o Memory conclui todas as metas vinculadas que estiverem programadas para aquele dia.</p></div></div>
