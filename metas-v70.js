...
(async()=>{
  ...
  function isWeeklyForm(){return $('#goal-schedule-mode')?.value==='semanal_flexivel'}
  function isMonthlyForm(){return $('#goal-schedule-mode')?.value==='mensal_flexivel'}
  function isFlexibleForm(){return isWeeklyForm()||isMonthlyForm()}
  function refreshScheduleMode(){
    const weekly=isWeeklyForm();
    const monthly=isMonthlyForm();
    const flex=weekly||monthly;
    const fixed=$('#fixed-days-card');
    const weeklyInfo=$('#weekly-flex-info');
    const monthlyInfo=$('#monthly-flex-info');
    const freq=$('#goal-frequency');
    if(fixed) fixed.hidden=flex;
    if(weeklyInfo) weeklyInfo.hidden=!weekly;
    if(monthlyInfo) monthlyInfo.hidden=!monthly;
    if(freq){
      if(weekly){freq.value='semanal';freq.disabled=true}
      else if(monthly){freq.value='mensal';freq.disabled=true}
      else freq.disabled=false;
    }
    const label=$('#goal-quantity-label');
    if(label) label.textContent=weekly?'Vezes por semana':monthly?'Vezes por mês':'Quantidade';
  ...
  $('#goal-form').onsubmit=async e=>{
    ...
    if(!flex&&!diasSemana.length){MMCDUI.toast('Escolha ao menos um dia ou use um modo flexível.');return}
    ...
    const obj={
      ...
      frequencia:weekly?'semanal':monthly?'mensal':$('#goal-frequency').value,nivelInglesPorDia,descricao:$('#goal-description').value.trim(),diasSemana,quantidade:qtd,
      metaSemanal:weekly?qtd:qtd,metaMensal:monthly?qtd:qtd,modoProgramacao:weekly?'semanal_flexivel':monthly?'mensal_flexivel':'dias',unidade:$('#goal-unit').value.trim(),inicioVigencia:$('#goal-start').value,
    ...
    setExtension(obj);
    try{await MMCD.salvar(d);await saveExtensions();close();render();MMCDUI.toast(weekly?'Meta semanal flexível salva':monthly?'Meta mensal flexível salva':'Meta salva')}
