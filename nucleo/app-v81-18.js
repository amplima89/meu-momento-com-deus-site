...
let indiceAtual = -1;
let temporizadorGeracaoDevocional = null;
let verificacoesGeracaoDevocional = 0;
...
function tituloVisivelSecao(titulo = "") {
    const nome =
        normalizarTexto(titulo);

    if (
        nome.includes("silencio")
        && nome.includes("reflexao")
    ) {
        return "🎯 Aplicação de hoje";
    }
...
function grupoDaSecao(titulo = "") {
    ...
    if (
        ...
        return {
            ...
            descricao: "Responder à Palavra em oração sincera, confissão, gratidão e entrega."
        };
    }
    ...
    // V80.2: se uma seção não pertence às quatro etapas principais,
    // ela não cria uma quinta etapa visual.
    return null;
}
...
function renderizarSecoesAgrupadas(secoes = []) {
    ...
    const grupos = [];
    const secoesSemGrupo = [];

    secoes.forEach(secao => {
        const grupo = grupoDaSecao(secao.titulo || "");

        if (!grupo) {
            secoesSemGrupo.push(secao);
            return;
        }
    ...
    const ordem = ["preparacao", "palavra", "resposta", "pratica"];
    grupos.sort((a, b) => ordem.indexOf(a.grupo.id) - ordem.indexOf(b.grupo.id));

    /*
     * V81.9 — acordeão exclusivo:
     * no máximo uma das quatro etapas fica aberta.
     * Se ainda não existe preferência salva, a primeira etapa abre.
     */
    let grupoAberto = "";
    ...
    const gruposHtml = grupos.map(({ grupo, secoes: itens }) => {
        const recolhido =
            grupo.id !== grupoAberto;
    ...
    // Preserva conteúdo real de uma seção não reconhecida, mas sem inventar
    // uma etapa chamada Continuidade/Complementos. Blocos técnicos vazios
    // (como cercas ```markdown) são descartados antes deste ponto.
    const avulsasHtml = secoesSemGrupo
        .filter(secao => {
            const linhas = secao.linhas
                .map(linha => linha.trim())
                .filter(Boolean);
            return Boolean(secao.titulo?.trim()) || linhas.length > 0;
        })
        .map(renderizarSecao)
        .join("");

    return `${gruposHtml}${avulsasHtml}`;
}
...
let temporizadorRetomarDestaques = null;

function pausaDestaques(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }

function assinaturaDestaques(destaques) {
    return JSON.stringify(normalizarDestaques(destaques).map(item => ({
        id: item.id, inicio: item.inicio, fim: item.fim, texto: item.texto
    })));
}

function garantirStatusDestaques() {
    const ferramentas = document.querySelector('.highlight-tools');
    if (!ferramentas) return null;
    let linha = ferramentas.querySelector('.highlight-sync-row');
    if (!linha) {
        linha = document.createElement('div');
        linha.className = 'highlight-sync-row';
        linha.innerHTML = `
          <span id="highlight-sync-status" class="highlight-sync-status" data-state="idle" aria-live="polite">Sincronização pronta</span>
          <button id="botao-sincronizar-destaques" class="highlight-sync-button" type="button">Sincronizar agora</button>`;
        ferramentas.appendChild(linha);
    }
    return linha.querySelector('#highlight-sync-status');
}

function atualizarStatusDestaques(estado, texto) {
    const status = garantirStatusDestaques();
    if (!status) return;
    status.dataset.state = estado || 'idle';
    status.textContent = texto || 'Sincronização pronta';
}

async function renovarSessaoDestaques() {
    try {
        const auth = window.MMCDSupabase?.auth;
        if (!auth?.refreshSession) return false;
        const { error } = await auth.refreshSession();
        if (error) { console.warn('Devocional: sessão não pôde ser renovada.', error); return false; }
        return true;
    } catch (erro) {
        console.warn('Devocional: erro ao renovar sessão.', erro); return false;
    }
}

async function executarOnlineDestaques(operacao, contexto='sincronizar') {
    let ultimoErro = null;
    for (let tentativa=1; tentativa<=3; tentativa+=1) {
        try { return await operacao(); }
        catch (erro) {
            ultimoErro = erro;
            console.warn(`Devocional: tentativa ${tentativa} falhou ao ${contexto}.`, erro);
            if (tentativa === 1) await renovarSessaoDestaques();
            if (tentativa < 3) await pausaDestaques(tentativa === 1 ? 500 : 1100);
        }
    }
    throw ultimoErro || new Error(`Não foi possível ${contexto}.`);
}

async function verificarPersistenciaDestaques(data, esperados) {
    const servidor = await executarOnlineDestaques(
        () => window.MMCD.listarDestaquesMeditacao(data),
        'confirmar os destaques no banco'
    );
    if (servidor === null) return false;
    return assinaturaDestaques(servidor) === assinaturaDestaques(esperados);
}
...
async function carregarDestaquesPersistidos() {
    const data = dataDestaquesMeditacao();
    if (!data) return;

    const token = ++tokenCargaDestaques;
    const locais = lerDestaquesLocais(data);
    destaquesMeditacaoAtuais = locais;
    aplicarDestaquesVisuais(locais);

    if (!window.MMCD?.listarDestaquesMeditacao) {
        atualizarStatusDestaques('local','Somente neste aparelho');
        return;
    }

    atualizarStatusDestaques('syncing','Sincronizando…');

    try {
        const respostaBanco = await executarOnlineDestaques(
            () => window.MMCD.listarDestaquesMeditacao(data),
            'carregar os destaques'
        );
        const possuiRegistroNoBanco = respostaBanco !== null;
        const banco = normalizarDestaques(respostaBanco || []);

        if (token !== tokenCargaDestaques || data !== dataDestaquesMeditacao()) return;

        let finais = banco;
        if (!possuiRegistroNoBanco && locais.length) {
            finais = locais;
            await executarOnlineDestaques(
                () => window.MMCD.substituirDestaquesMeditacao(data, locais),
                'enviar os destaques antigos'
            );
            if (!(await verificarPersistenciaDestaques(data, locais))) {
                throw new Error('O banco não confirmou os destaques enviados.');
            }
            window.MMCDUI?.toast?.('Seus destaques antigos foram sincronizados com o banco.');
        }

        destaquesMeditacaoAtuais = finais;
        salvarCacheLocalDestaques(data, finais);
        aplicarDestaquesVisuais(finais);
        atualizarStatusDestaques('online','Sincronizado entre aparelhos');
    } catch (erro) {
        console.error('Não foi possível carregar os destaques do banco.', erro);
        atualizarStatusDestaques('error','Falha de sincronização');
        if (locais.length) {
            window.MMCDUI?.toast?.('Destaques carregados deste aparelho; toque em Sincronizar agora para tentar novamente.');
        }
    }
}


function salvarDestaquesPersistidos(destaques, data = dataDestaquesMeditacao()) {
    const normalizados = normalizarDestaques(destaques);
    destaquesMeditacaoAtuais = normalizados;
    salvarCacheLocalDestaques(data, normalizados);

    if (!data || !window.MMCD?.substituirDestaquesMeditacao) {
        atualizarStatusDestaques('local','Salvo somente neste aparelho');
        return Promise.resolve(false);
    }

    atualizarStatusDestaques('syncing','Salvando no banco…');

    filaPersistenciaDestaques = filaPersistenciaDestaques
      .catch(() => undefined)
      .then(async () => {
        for (let tentativa=1; tentativa<=3; tentativa+=1) {
            try {
                await executarOnlineDestaques(
                    () => window.MMCD.substituirDestaquesMeditacao(data, normalizados),
                    'salvar os destaques'
                );
                if (!(await verificarPersistenciaDestaques(data, normalizados))) {
                    throw new Error('O banco respondeu, mas ainda não devolveu as mesmas marcações.');
                }
                atualizarStatusDestaques('online','Sincronizado entre aparelhos');
                return true;
            } catch (erro) {
                if (tentativa >= 3) throw erro;
                await pausaDestaques(tentativa * 650);
            }
        }
        return false;
      });

    return filaPersistenciaDestaques.catch(erro => {
        console.error('Não foi possível salvar os destaques no banco.', erro);
        atualizarStatusDestaques('error','Salvo localmente · sincronização pendente');
        window.MMCDUI?.toast?.('O destaque ficou salvo neste aparelho, mas ainda não sincronizou.');
        return false;
    });
}
...
function sincronizarDestaquesAoRetomar() {
    clearTimeout(temporizadorRetomarDestaques);
    if (document.hidden || !dataDestaquesMeditacao()) return;
    const selecao = window.getSelection?.();
    if (selecao && !selecao.isCollapsed && String(selecao).trim()) return;
    temporizadorRetomarDestaques = setTimeout(() => carregarDestaquesPersistidos(), 260);
}

document.addEventListener('visibilitychange', () => { if (!document.hidden) sincronizarDestaquesAoRetomar(); });
window.addEventListener('focus', sincronizarDestaquesAoRetomar);
window.addEventListener('online', sincronizarDestaquesAoRetomar);
document.addEventListener('click', evento => {
    if (evento.target.closest?.('#botao-sincronizar-destaques')) {
        evento.preventDefault(); carregarDestaquesPersistidos();
    }
});
...
function existeDevocionalHoje() {
    const hoje =
        dataHojeISO();

    return meditacoes.some(
        item =>
            item?.data === hoje
    );
}
...
function chaveSolicitacaoGeracaoHoje() {
    return (
        "meditacao_geracao_"
        + dataHojeISO()
    );
...
function geracaoDevocionalHtml() {
    return `
        <aside class="meditation-generation-card">
            <div class="meditation-generation-card__icon">⚡</div>

            <div class="meditation-generation-card__copy">
                <span>DEVOCIONAL DE HOJE AUSENTE</span>
                <strong>Gerar devocional de hoje</strong>

                <p data-generation-message>
                    Você pode solicitar pelo celular. A chave da IA continua protegida no seu PC:
                    quando ele estiver ligado e conectado, a rotina processa o pedido automaticamente.
                </p>
            </div>

            <button
                type="button"
                class="meditation-generation-card__button"
                data-action="request-today-meditation"
            >
...
function atualizarEstadoGeracaoDevocional(
    estado,
    mensagem
) {
    const card =
        document.querySelector(
            ".meditation-generation-card"
        );
...
async function sessaoAtualGeracao() {
    const cliente =
        window.MMCDSupabase;
...
async function registrarSolicitacaoGeracao() {
    const {
        cliente,
        sessao
    } =
        await sessaoAtualGeracao();
...
async function consultarStatusSolicitacaoGeracao() {
    const {
        cliente,
        sessao
    } =
...
let temporizadorSincronizacaoDevocional = null;
let sincronizacaoDevocionalEmAndamento = false;
let ultimaSincronizacaoDevocional = 0;


function assinaturaListaDevocionais(lista) {
    return (Array.isArray(lista) ? lista : [])
        .map(item => `${item?.data || ""}:${String(item?.markdown || "").length}`)
        .join("|");
}


async function sincronizarListaDevocionais({
    abrirHojeSeNovo = true,
    silencioso = true,
} = {}) {
    if (
        sincronizacaoDevocionalEmAndamento
        || !window.MMCD?.listarMeditacoes
    ) {
        return false;
    }
    ...
    // Evita chamadas duplicadas quando focus/visibility disparam juntos.
    if (
        agora - ultimaSincronizacaoDevocional < 1500
    ) {
        return false;
    }

    sincronizacaoDevocionalEmAndamento = true;
    ultimaSincronizacaoDevocional = agora;

    try {
        const antes = assinaturaListaDevocionais(
            meditacoes
        );

        const novas = await window.MMCD
            .listarMeditacoes();
        ...
        const normalizadas = novas
            .filter(
                item =>
                    item
                    && item.data
                    && typeof item.markdown
                        === "string"
            )
            .sort(
                (a, b) =>
        ...
        if (
            ...
            window.MMCDUI?.toast?.(
                indiceHoje >= 0
                    ? "Devocional de hoje sincronizado."
        ...
        console.warn(
            "Devocional: sincronização automática indisponível.",
            erro
        );
...
function configurarSincronizacaoAutomaticaDevocional() {
    const sincronizarAoRetomar = () => {
        if (document.hidden) {
            return;
        }

        sincronizarListaDevocionais({
            abrirHojeSeNovo: true,
            silencioso: true,
        });
    };
    ...
    clearInterval(
        temporizadorSincronizacaoDevocional
    );
    ...
    // Captura o caso em que a página abriu praticamente junto da publicação.
    setTimeout(
        sincronizarAoRetomar,
        2500
    );
...
async function recarregarDevocionaisDepoisDaGeracao() {
    if (
        !window.MMCD
        ?.listarMeditacoes
    ) {
...
async function verificarGeracaoSolicitada() {
    verificacoesGeracaoDevocional += 1;

    try {
        const carregou =
            await recarregarDevocionaisDepoisDaGeracao();
        ...
        const status =
            await consultarStatusSolicitacaoGeracao();

        if (
            status?.status === "erro"
        ) {
            atualizarEstadoGeracaoDevocional(
                "error",
                status.mensagem
                    || "A rotina do PC encontrou um erro."
            );
        ...
        if (
            ...
            atualizarEstadoGeracaoDevocional(
                "waiting",
                "Seu PC recebeu o pedido e está gerando o devocional."
            );
        }
    } catch (erro) {
        console.warn(
            "Devocional: geração ainda não confirmada.",
            erro
        );
    }

    if (
        verificacoesGeracaoDevocional
        >= 40
    ) {
        atualizarEstadoGeracaoDevocional(
            "waiting",
            "O pedido continua salvo. Se o PC estiver desligado, será processado quando você entrar no Windows."
        );
        return;
    }
    ...
    temporizadorGeracaoDevocional =
        setTimeout(
            verificarGeracaoSolicitada,
            15000
        );
}


async function solicitarGeracaoDevocionalHoje() {
    if (
        existeDevocionalHoje()
    ) {
        await recarregarDevocionaisDepoisDaGeracao();
    ...
    atualizarEstadoGeracaoDevocional(
        "requesting",
        "Registrando o pedido no Memory…"
    );

    try {
        await registrarSolicitacaoGeracao();
        ...
        atualizarEstadoGeracaoDevocional(
            "waiting",
            "Pedido enviado. Se o PC estiver ligado, a rotina verifica solicitações a cada 15 minutos. Se estiver desligado, processará no próximo login."
        );
        ...
        console.error(
            "Devocional: falha ao solicitar geração.",
            erro
        );

        atualizarEstadoGeracaoDevocional(
            "error",
            "Não consegui registrar o pedido agora. Verifique sua conexão e tente novamente."
        );
    }
}
...
document.addEventListener(
    ...
    evento => {
        if (
            ...
            evento.preventDefault();
            solicitarGeracaoDevocionalHoje();
        }
    }
);
...
function renderizarVazio() {
    ...
    elementos.conteudo.innerHTML = `
        ${geracaoDevocionalHtml()}

        <p class="estado-vazio">
            Ainda não há uma devocional publicada para abrir.
        </p>
    `;
...
function renderizarMeditacao() {
    ...
    const markdownLimpo = String(meditacao.markdown || "")
        ...
        .filter(linha => {
            ...
            // Remove resíduos de conflito Git.
            if (/^(<{7}|={7}|>{7})(\s.*)?$/.test(limpa)) {
                return false;
            }

            // Algumas meditações chegaram embrulhadas em ```markdown ... ```.
            // A interface já interpreta Markdown; exibir essas cercas como texto
            // não faz sentido e criava o antigo bloco "Complementos".
            if (/^```(?:markdown|md)?\s*$/i.test(limpa)) {
                return false;
            }
...
function definirEstadoGrupoMeditacao(
    grupo,
    recolhido
) {
    if (!grupo) {
    ...
    const botao =
        grupo.querySelector(
            "[data-meditacao-grupo-toggle]"
        );

    const conteudo =
        grupo.querySelector(
            ".meditacao-grupo__conteudo"
        );
...
elementos.conteudo?.addEventListener(
    ...
    evento => {
        ...
        /*
         * Quando uma etapa abre, todas as outras fecham.
         * Isso evita duas ou mais das quatro guias expandidas.
         */
        if (vaiAbrir) {
            elementos.conteudo
                .querySelectorAll(
        ...
        /*
         * Clicar na etapa que já está aberta pode recolhê-la.
         * Continua valendo a regra: nunca haverá mais de uma aberta.
         */
        definirEstadoGrupoMeditacao(
            grupo,
            true
...
async function iniciar() {
    ...
    if (!meditacoes.length) {
        renderizarVazio();
        configurarSincronizacaoAutomaticaDevocional();
        return;
    }
...
iniciar().catch(erro => {
    console.error('Erro ao abrir o livro:', erro);
    elementos.conteudo.innerHTML = `
      <p class="estado-vazio">
        Não foi possível carregar esta devocional agora.
        Verifique a conexão e tente novamente.
      </p>`;
    atualizarStatusDestaques('error','Sincronização indisponível');
    window.MMCDUI?.toast?.('O Devocional não conseguiu carregar os dados online. Tente novamente em alguns instantes.',5200);
});
