...
window.MMCDShell=async function(active){
 ...
 const mobileMemoryHtml=`
  <a class="sidebar-link sidebar-memory-mobile ${active==='memory'?'active':''}" href="memory.html?v=20260815-v76" aria-label="Abrir Memory">
   <span class="sidebar-link__icon sidebar-memory-mobile__icon"><img src="assets/imagens/memory-mark-official-v81-11-3.png?v=20260822-v81-11-3" alt=""></span>
  ...
  <aside class="sidebar sidebar-v24">
   <a class="sidebar-brand" href="memory.html?v=20260815-v76" aria-label="Abrir propósito e essência do Memory" title="Abrir Memory">
    <span class="sidebar-brand__mark">
     <img src="assets/imagens/memory-mark-official-v81-11-3.png?v=20260822-v81-11-3" alt="Memory">
   ...
   <nav class="sidebar-nav" aria-label="Navegação principal">
    ...
    <div class="sidebar-settings sidebar-care-group ${careActive?'open':''}">
     <div class="sidebar-care-head">
      <a href="mapa-cuidado.html?v=20260816-v80-6" class="sidebar-link sidebar-care-link ${careActive?'active':''}" title="Abrir Mapa de Cuidado">
       ...
       <span class="sidebar-mobile-label">Cuidado</span>
      </a>
      <button type="button" class="sidebar-care-disclosure" aria-expanded="${careActive?'true':'false'}" aria-controls="sidebar-care-menu" aria-label="${careActive?'Recolher':'Expandir'} Cuidado" title="${careActive?'Recolher':'Expandir'} Cuidado">
     ...
     <div class="sidebar-subnav ${careActive?'open':''}" id="sidebar-care-menu" ${careActive?'':'hidden'}>
      <a class="sidebar-subnav__link ${active==='mapa-cuidado'||active==='arvore-da-vida'?'active':''}" href="mapa-cuidado.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Mapa de Cuidado</strong><small>Visão integral</small></span></a>
      <a class="sidebar-subnav__link ${active==='aniversarios'?'active':''}" href="aniversarios.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Aniversariantes</strong><small>Datas e gestos de cuidado</small></span></a>
      <a class="sidebar-subnav__link ${active==='boas-acoes'?'active':''}" href="boas-acoes.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Boas Ações</strong><small>Cuidado colocado em prática</small></span></a>
      <a class="sidebar-subnav__link ${active==='oracoes'?'active':''}" href="oracoes.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Minhas Orações</strong><small>Pedidos e Memórias de Deus</small></span></a>
      <a class="sidebar-subnav__link ${active==='registro-rapido'?'active':''}" href="registro-rapido.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Registro rápido</strong><small>Memória de curto prazo</small></span></a>
      <a class="sidebar-subnav__link ${active==='circulo-cuidado'?'active':''}" href="circulo-cuidado.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Relacionamentos</strong><small>Presença com quem importa</small></span></a>
      <a class="sidebar-subnav__link ${active==='testemunhos'?'active':''}" href="testemunhos.html?v=20260816-v80-6"><span class="sidebar-subnav__dot"></span><span><strong>Testemunhos</strong><small>Memórias do que Deus fez</small></span></a>
     </div>
    </div>
    ${navAfterCareHtml}
 ...
 document.querySelectorAll('.app-topbar__title').forEach(el=>{el.innerHTML='<a class="memory-topbar-brand" href="memory.html?v=20260815-v76" aria-label="Abrir Memory"><img src="assets/imagens/memory-mark-official-v81-11-3.png?v=20260822-v81-11-3" alt=""><span>Memory</span></a>'});
 const memoryTitles={
  memory:'Memory',
 ...
 if(!document.querySelector('#memory-care-group-v806-style')){
  const careGroupStyle=document.createElement('style');
  careGroupStyle.id='memory-care-group-v806-style';
  careGroupStyle.textContent=`
   .sidebar-care-head{position:relative}
   .sidebar-care-head>.sidebar-care-link{padding-right:38px}
   .sidebar-care-disclosure{
    position:absolute;right:7px;top:50%;transform:translateY(-50%);
    z-index:3;width:28px;height:28px;display:grid;place-items:center;
   ...
   .sidebar-care-disclosure:hover{
    background:var(--nav-hover);border-color:var(--nav-line);color:var(--nav-text);
   }
   .sidebar-care-disclosure .sidebar-settings__chevron{
    display:block;transition:transform .18s ease;
   }
   .sidebar-care-group.open .sidebar-care-disclosure .sidebar-settings__chevron{
    transform:rotate(90deg);
   }
   .sidebar-care-group .sidebar-subnav{
    margin-top:5px;margin-left:30px;padding-left:10px;
    border-left:1px solid var(--nav-line);
   }
   .sidebar-care-group:not(.open) .sidebar-subnav{display:none!important}
   @media(max-width:760px){
    .sidebar-care-head>.sidebar-care-link{padding-right:0}
    .sidebar-care-disclosure{
     right:4px;top:6px;transform:none;width:22px;height:22px;
     border-radius:7px;background:color-mix(in srgb,var(--nav-bg) 78%,transparent);
    }
   }
  `;
  document.head.append(careGroupStyle);
 }

 // MEMORY_CARE_NAV_V78_START
 const careGroup=document.querySelector('.sidebar-care-group');
 const careLink=careGroup?.querySelector('.sidebar-care-link');
 const careToggle=careGroup?.querySelector('.sidebar-care-disclosure');
 ...
 const setCareOpen=(next,{persist=true}={})=>{
  ...
  careToggle.setAttribute('aria-label',`${safe?'Recolher':'Expandir'} Cuidado`);
  careToggle.title=`${safe?'Recolher':'Expandir'} Cuidado`;
 ...
 if(careGroup&&careToggle&&careMenu){
  ...
  // Clicar no nome Cuidado continua abrindo o Mapa de Cuidado.
  careLink?.addEventListener('click',()=>{
   localStorage.setItem('memory:sidebar:care-open','1');
  });

  // A seta controla o agrupamento sem navegar para outra página.
  careToggle.addEventListener('click',event=>{
   event.preventDefault();
   event.stopPropagation();
   if(isMobileNav()){
 ...
 if(logo){
  ...
  const fallback=()=>{
   if(logo.dataset.memoryFallbackTried!=='1'){
    ...
    logo.src='assets/imagens/memory-mark-official-v81-11-3.png?v=20260822-v81-11-3';
    return;
   }
