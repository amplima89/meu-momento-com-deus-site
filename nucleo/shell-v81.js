...
window.MMCDShell=async function(active){
  ...
  <aside class="sidebar sidebar-v24">
   ...
   <nav class="sidebar-nav" aria-label="Navegação principal">
    ...
    <div class="sidebar-settings sidebar-care-group ${careActive?'open':''}">
     <button type="button" class="sidebar-link sidebar-settings__toggle sidebar-care-toggle ${careActive?'active':''}" aria-expanded="${careActive?'true':'false'}" aria-controls="sidebar-care-menu" title="Cuidado">
     ...
     <div class="sidebar-subnav ${careActive?'open':''}" id="sidebar-care-menu" ${careActive?'':'hidden'}>
      <a class="sidebar-subnav__link ${active==='mapa-cuidado'||active==='arvore-da-vida'?'active':''}" href="mapa-cuidado.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Mapa de Cuidado</strong><small>Visão integral</small></span></a>
      <a class="sidebar-subnav__link ${active==='aniversarios'?'active':''}" href="aniversarios.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Aniversariantes</strong><small>Datas e gestos de cuidado</small></span></a>
      <a class="sidebar-subnav__link ${active==='boas-acoes'?'active':''}" href="boas-acoes.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Boas Ações</strong><small>Cuidado colocado em prática</small></span></a>
      <a class="sidebar-subnav__link ${active==='oracoes'?'active':''}" href="oracoes.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Minhas Orações</strong><small>Pedidos e Memórias de Deus</small></span></a>
      <a class="sidebar-subnav__link ${active==='registro-rapido'?'active':''}" href="registro-rapido.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Registro rápido</strong><small>Memória de curto prazo</small></span></a>
      <a class="sidebar-subnav__link ${active==='circulo-cuidado'?'active':''}" href="circulo-cuidado.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Relacionamentos</strong><small>Presença com quem importa</small></span></a>
      <a class="sidebar-subnav__link ${active==='testemunhos'?'active':''}" href="testemunhos.html?v=20260816-v80-6-1"><span class="sidebar-subnav__dot"></span><span><strong>Testemunhos</strong><small>Memórias do que Deus fez</small></span></a>
     </div>
    </div>
    ${navAfterCareHtml}
 ...
 if(!document.querySelector('#memory-care-group-v8061-style')){
  const careGroupStyle=document.createElement('style');
  careGroupStyle.id='memory-care-group-v8061-style';
  careGroupStyle.textContent=`
   .sidebar-care-group .sidebar-care-toggle{width:100%;text-align:left}
   .sidebar-care-group .sidebar-care-toggle .sidebar-settings__chevron{
    display:block;transition:transform .18s ease;
   }
   .sidebar-care-group.open .sidebar-care-toggle .sidebar-settings__chevron{
   ...
   .sidebar-care-group:not(.open) .sidebar-subnav{
 ...
 const careToggle=careGroup?.querySelector('.sidebar-care-toggle');
 ...
 if(careGroup&&careToggle&&careMenu){
  // Respeita o estado salvo. Antes, careActive forçava o grupo a ficar aberto
  // e fazia parecer que o agrupamento não funcionava.
  const stored=localStorage.getItem('memory:sidebar:care-open');
  const initialOpen=stored===null?careActive:stored==='1';
  setCareOpen(initialOpen,{persist:false});
  ...
  careToggle.addEventListener('click',event=>{
   ...
   if(!careActive){
    // Ao entrar em Cuidado por outra área, abre o grupo e mostra o Mapa.
    localStorage.setItem('memory:sidebar:care-open','1');
    window.location.href='mapa-cuidado.html?v=20260816-v80-6-1';
    return;
   }

   // Já dentro de Cuidado: o card inteiro abre/recolhe o agrupamento.
