...
(async()=>{
  ...
  function isFlexibleForm(){return $('#goal-schedule-mode')?.value==='semanal_flexivel'}
  function refreshScheduleMode(){
    ...
    if(info) info.hidden=!flex;
    if(freq){
      if(flex){freq.value='semanal';freq.disabled=true}
      else freq.disabled=false;
    }
    const label=$('#goal-quantity-label');
    if(label) label.textContent=flex?'Vezes por semana':'Quantidade';
    const unit=$('#goal-unit');
    if(flex&&unit&&!unit.value) unit.placeholder='vezes, treinos, sessões...';
  ...
  function open(m){
    ...
    $('#goal-quantity').value=extra?.metaSemanal||m?.metaSemanal||m?.quantidade||1;
  ...
  function levelsText(m){
    if(m.modoProgramacao==='semanal_flexivel') return '';
  ...
  function render(){
    ...
    $('#goals-list').innerHTML=list.map(m=>{
      ...
      return `<article class="goal-item ${m.ativa?'':'inactive'}">
        ...
        <div class="goal-report-cell goal-schedule-cell" data-label="Programação"><strong>${MMCDUI.esc(freq(m))}</strong><small>${MMCDUI.esc(dayText(m))}</small>${m.modoProgramacao==='semanal_flexivel'?'<span class="weekly-flex-pill">↻ Flexível</span>':''}${levelDetails?`<small class="english-level-summary">${MMCDUI.esc(levelDetails)}</small>`:''}</div>
  ...
  $('#goal-form').onsubmit=async e=>{
    ...
    if(!flex&&!diasSemana.length){MMCDUI.toast('Escolha ao menos um dia ou use Semanal flexível.');return}
    ...
    const obj={
      ...
      frequencia:flex?'semanal':$('#goal-frequency').value,nivelInglesPorDia,descricao:$('#goal-description').value.trim(),diasSemana,quantidade:qtd,
      metaSemanal:flex?qtd:qtd,modoProgramacao:flex?'semanal_flexivel':'dias',unidade:$('#goal-unit').value.trim(),inicioVigencia:$('#goal-start').value,
    ...
    setExtension(obj);
    try{await MMCD.salvar(d);await saveExtensions();close();render();MMCDUI.toast(flex?'Meta semanal flexível salva':'Meta salva')}
