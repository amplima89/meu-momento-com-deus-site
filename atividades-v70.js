...
(async () => {
  ...
  function monthlyState(meta, date) {
    ...
    const failed = ended && total === 0;
    return {
      ...bounds, target, total, remaining: Math.max(0, target - total), pct, ended, complete, partial, failed, completionDate,
      selectedCount: executionsOnDate(meta.id, date)
    };
  }

  function monthlyMetasFor(date) {
    return (d.metas || [])
  ...
  function monthlyStatusText(state) {
    ...
    if (state.total > 0) return `${state.total} de ${state.target} · ${state.pct}% · falta${state.remaining === 1 ? "" : "m"} ${state.remaining} até o fim do mês`;
  ...
  function renderMonthlyGoal(meta) {
    ...
    return `<article class="weekly-flex-goal monthly-flex-goal ${stateClass}">
      <div class="weekly-flex-goal__top">
        <span class="weekly-flex-goal__icon">${MMCDUI.esc(meta.icone || "◷")}</span>
        <div class="weekly-flex-goal__copy">
          <div class="weekly-flex-goal__title"><strong>${MMCDUI.esc(meta.nome || "Meta mensal")}</strong><span>${state.pct}%</span></div>
          <small>${MMCDUI.esc(monthlyStatusText(state))}</small>
        </div>
      </div>
      <div class="weekly-flex-progress"><i style="width:${state.pct}%"></i></div>
      <div class="weekly-flex-goal__footer">
        <span>${state.total}/${state.target} ${MMCDUI.esc(unit)} neste mês</span>
        <div class="weekly-flex-actions">
  ...
  async function changeMonthlyExecution(metaId, delta) {
    ...
    current = Math.max(0, current + delta);
    MMCD.setRegistro(d, selected, metaId, {
      concluida: current > 0, abonada: false, valor: current, texto: "", observacao: previous?.observacao || "", origem: "mensal_flexivel"
    });
    render();
    setStatus("Salvando meta mensal...", "saving");
    try {
      d = await MMCD.salvarRegistroAtividade(d, selected, metaId);
      render();
      const updated = monthlyState(meta, selected);
      setStatus(updated.complete ? "Meta mensal concluída" : "Progresso mensal salvo", "success", true);
    } catch (error) {
      restoreRecord(metaId, previousState);
      render();
      showError(error, "Não foi possível salvar a meta mensal.");
    }
  }
  ...
  function renderDay() {
    ...
    const monthlyFlexible = monthlyMetasFor(selected);
    const monthlyHtml = monthlyFlexible.length ? `
      <section class="weekly-flex-section monthly-flex-section">
        <div class="weekly-flex-section__head"><div><span>MÊS FLEXÍVEL</span><strong>Você decide quando neste mês</strong></div><small>Só fecha como não concluída quando o mês terminar.</small></div>
    ...
    $("#daily-goals").innerHTML = weeklyHtml + monthlyHtml + regularHtml || '<div class="empty-day">Nenhuma meta programada para este dia.</div>';
