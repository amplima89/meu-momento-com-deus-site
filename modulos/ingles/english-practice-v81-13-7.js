...
window.MMCDEnglishPractice = (() => {
    ...
    host.innerHTML = `
      <div class="english-context-practice">
        ...
        <footer class="english-context-finish">
          <div>
            <strong>${finished ? "Roteiro pronto para concluir." : `Faltam ${QUESTIONS_PER_ROUND-done} respostas.`}</strong>
            <span>${totalRounds} roteiro(s) já concluído(s) no histórico.</span>
          </div>
  ...
  async function render({host,data,db,usuario}) {
    ...
    try {
      ...
      const roundNo = session.rounds.filter(round => round?.completed).length;
      ...
      host.addEventListener("click",async event => {
        ...
        host.innerHTML = `
          <div class="english-context-complete">
            <p class="eyebrow">Prática concluída</p>
            <h2>Você sustentou uma situação real em inglês.</h2>
            <p>O objetivo aqui não era adivinhar uma alternativa correta. Era organizar suas próprias ideias e responder de forma compreensível e natural.</p>

            <div class="english-context-complete__stats">
              <div><strong>${QUESTIONS_PER_ROUND}</strong><span>respostas produzidas</span></div>
              <div><strong>${script.label}</strong><span>contexto praticado</span></div>
            </div>

            <div class="english-context-complete__actions">
              <button type="button" class="btn primary" data-context-new>
                Nova situação
              </button>
              <button type="button" class="btn" data-context-stop>
                Encerrar por hoje
              </button>
            </div>
          </div>
        ...
        host.onclick = event2 => {
          if(event2.target.closest("[data-context-new]")) {
            window.MMCDEnglishPractice.render({
              host,
              data:date,
              db,
              usuario
          ...
          if(event2.target.closest("[data-context-stop]")) {
            ...
            toast("Prática concluída por hoje.");
          }
        };
      });
