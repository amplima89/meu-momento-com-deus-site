...
window.MMCDEnglishExperience = (() => {
  ...
  async function render({container,data,db,usuario}) {
    ...
    if(window.MMCDEnglishReadingRecorderV81171?.init) {
      try {
        await window.MMCDEnglishReadingRecorderV81171.init({
          data,
          db,
          usuario
        });
    ...
    if(window.MMCDEnglishFinalCorrectionV81171?.render) {
      await window.MMCDEnglishFinalCorrectionV81171.render({
        container,
        data,
        db,
        usuario
