...
window.MMCDEnglishFinalCorrectionV81171 = (() => {
  const FINAL_KEY="ingles_correcao_final_v81_17_1";
  ...
  function practicePlanHtml(plan={}) {
    ...
    const priorities=Array.isArray(plan.priorities)
      ? plan.priorities
      : [];

    if(!priorities.length && !plan.diagnosis && !plan.challenge) {
      return "";
    }

    return `
      <section class="english-final-practice-plan">
        <header>
          <span>COMO PRATICAR ESSAS CORREÇÕES</span>
          <h4>${esc(plan.diagnosis || "Treino baseado nos seus erros de hoje")}</h4>
          <p>Sem múltipla escolha e sem completar lacunas. A ideia é produzir inglês em situações reais usando os padrões que precisam melhorar.</p>
        </header>

        <div class="english-final-practice-plan__items">
          ${priorities.map((item,index)=>`
            <article>
              <div class="english-final-practice-plan__number">${index+1}</div>
              <div>
                <span>${esc(item.skill || "Ponto de melhoria")}</span>
                ${item.why ? `<p class="why">${esc(item.why)}</p>` : ""}
                ${item.pattern ? `
                  <div class="pattern">
                    <b>PADRÃO PARA TREINAR</b>
                    <p>${esc(item.pattern)}</p>
                  </div>
                ` : ""}
                ${item.prompt ? `
                  <div class="prompt">
                    <b>RESPONDA EM INGLÊS</b>
                    <p>${esc(item.prompt)}</p>
                  </div>
                ` : ""}
                ${item.target ? `<small>${esc(item.target)}</small>` : ""}
              </div>
            </article>
          `).join("")}
        </div>
        ...
        ${plan.challenge ? `
          <div class="english-final-practice-plan__challenge">
            <span>DESAFIO CURTO</span>
            <p>${esc(plan.challenge)}</p>
          </div>
        ` : ""}
      </section>
    `;
  ...
  function renderResult(state) {
    ...
    host.innerHTML=`
      <section class="english-final-clean english-final-clean--result">
        ...
        ${practicePlanHtml(analysis.practicePlan)}
    ...
    host.innerHTML=`
      <section class="english-final-clean">
        ...
        <p class="english-final-clean__helper">
          A IA lê o texto completo e, quando houver gravação, também avalia sua leitura em voz alta. Depois corrige suas respostas da Conversa e da Prática em contexto.
        </p>
      </section>
    `;
