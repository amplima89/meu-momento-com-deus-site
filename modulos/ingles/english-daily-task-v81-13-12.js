...
window.MMCDEnglishDailyV8113 = (() => {
            ...
            <label class="english-daily-question">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Responda em inglês. A correção será feita uma única vez no final."
                ></textarea>
              </div>
            </label>
          `).join("")}
  ...
  function renderTask(task,reviews,draft,remoteDay) {
    ...
    const ready=["pronto","pendente","processando","corrigida"].includes(remoteDay?.status);
    ...
    return `
      <div class="english-daily-v8113" data-english-daily-task>
        ...
        <section class="english-daily-section">
          ...
          <div class="english-daily-writing">
            ...
            <textarea
              rows="8"
              data-daily-writing
              placeholder="Escreva sua resposta em inglês. A IA corrige tudo junto no final do inglês."
        ...
        <section class="english-daily-finish">
          <div>
            <span>PREPARAR PARA A CORREÇÃO FINAL</span>
            <strong>${ready
              ? "Esta parte está salva"
              : error
                ? "Não foi possível salvar esta parte"
                : "Conclua e salve suas respostas"
            }</strong>
            <p>${ready
              ? "Compreensão, estruturas, writing e speaking serão corrigidos juntos no final do inglês."
              : error
                ? esc(remoteDay?.error || "Tente salvar novamente.")
                : "Aqui não há correção da IA. Primeiro você termina todas as partes; depois faz uma única correção no final."
            }</p>
          </div>
          ...
          <button
            ...
            data-daily-finish
          >
            ${ready ? "Atualizar respostas salvas" : error ? "Tentar salvar novamente" : "Salvar esta parte do inglês"}
          </button>
        </section>

        <div data-daily-analysis></div>
      </div>
    `;
  }
  ...
  function validate(root,task,reviews) {
    ...
    const savedAudioKey=
      current?.remoteDay?.speaking?.key
      || current?.remoteDay?.speaking?.audioKey
      || "";

    if(!audioBlob && !savedAudioKey) {
  ...
  function progressStatus() {
    ...
    const values=[
      ...Object.values(answers.comprehension || {}),
      ...Object.values(answers.structures || {}),
      answers.writing || ""
    ];
    ...
    const saved=[
      ...
      "processando",
      "corrigida"
    ].includes(current?.remoteDay?.status);
  ...
  function emitProgress() {
    const detail=progressStatus();

    document.dispatchEvent(
      new CustomEvent("memory:english-part-progress",{
        detail:{
          part:"daily",
          ...detail
        }
  ...
  async function submit() {
    ...
    try {
      ...
      if(button) {
        ...
        button.textContent="Atualizar respostas salvas";
      }

      toast("Parte salva. A IA vai corrigir tudo junto no final do inglês.");

      document.dispatchEvent(
        new CustomEvent("memory:english-part-saved",{
          detail:{part:"daily",date}
        })
      );
      ...
      if(button) {
        button.disabled=false;
        button.textContent="Salvar esta parte do inglês";
      }

      toast("Não foi possível salvar esta parte do inglês.");
    }
  }
