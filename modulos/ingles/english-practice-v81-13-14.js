...
window.MMCDEnglishPractice = (() => {
  ...
  function answerText(value) {
    ...
    if(typeof value === "string") {
      return value.trim() === "[object Object]"
        ? ""
        : value;
    }
  ...
  function answerObject(value,index,question=null) {
    if(value && typeof value === "object" && !Array.isArray(value)) {
      return {
        id:String(value.id || `practice-${index+1}`),
        prompt:String(value.prompt || question?.prompt || ""),
  ...
  function responseState(session,roundNo,script) {
    ...
    if(savedRound?.completed) {
      const raw=Array.isArray(savedRound.answers)
        ? savedRound.answers
        : [];

      return {
        answers:raw.map(answerText),
        answerObjects:raw.map((value,index)=>
          answerObject(value,index,script?.questions?.[index])
        ),
        completed:true,
        savedRound
      };
      ...
      host.addEventListener("click",async event => {
        ...
        liveStatus={
          ...
          answers:script.questions.map((question,index)=>({
            id:`practice-${index+1}`,
            prompt:question.prompt,
            answer:String(state.answers[index] || "").trim()
          }))
        };
  ...
  async function renderWithStatus(options) {
    ...
    try {
      ...
      liveStatus={
        ...
        label:script.label,
        completed:Boolean(saved.completed),
        answers:Array.isArray(saved.answerObjects)
          ? saved.answerObjects.filter(item=>String(item?.answer || "").trim())
          : []
      };
    } catch(error) {
