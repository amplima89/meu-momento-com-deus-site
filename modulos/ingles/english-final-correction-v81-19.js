...
window.MMCDEnglishFinalCorrectionV8119 = (() => {
  ...
  function weeklyReviewMode() {
    return Boolean(document.querySelector("[data-english-weekly-review]"));
  }
  ...
  async function conversationFromSavedConfig() {
    try {
      const store=await readConfig("ingles_conversas_v1");
      const sessions=Array.isArray(store?.sessions) ? store.sessions : [];
      const session=sessions.find(item=>String(item?.date || "")===String(ctx?.data || ""));

      if(session && Array.isArray(session.answers)) {
        const answers=session.answers
          .filter(item=>cleanAnswer(item?.text || item?.userAnswer || item?.answer))
          .sort((a,b)=>Number(a?.index || 0)-Number(b?.index || 0))
          .map((item,index)=>({
            id:`conversation-${index+1}`,
            prompt:String(item?.prompt || item?.question || `Question ${index+1}`).trim(),
            userAnswer:cleanAnswer(item?.text || item?.userAnswer || item?.answer)
          }));

        if(answers.length) return answers.slice(0,10);
      }

      // Compatibilidade apenas para sessões antigas que ainda não guardavam
      // prompt/texto dentro de ingles_conversas_v1.
      const {data,error}=await ctx.db
        .from("configuracoes_usuario")
        .select("chave,valor")
        .eq("user_id",ctx.usuario.id);
      ...
      for(const row of rows.filter(row=>keyLooksLikeConversation(row?.chave))) {
        const value=row?.valor;
        if(String(value?.date || "")!==String(ctx?.data || "")) continue;
        const recovered=extractAnswerPairs(value,[]);
        if(recovered.length) return recovered;
      }
    } catch(error) {
      console.warn("Inglês: não foi possível recuperar a Conversa salva da data selecionada.",error);
    }
  ...
  async function readingContext() {
    const oral=window.MMCDEnglishReadingRecorderV81175?.status?.() || null;
  ...
  function clearInlineFeedback() {
    document.querySelectorAll("[data-english-ai-inline-v81176]")
      .forEach(node=>node.remove());
  }

  function conversationQuestionBlock(index) {
    const root=document.querySelector("#english-conversation-host");
    if(!root) return null;

    return root.querySelector(
      `[data-conversation-step="${index}"]`
    );
  }

  function conversationInlineHtml(item,index) {
    const status=String(item?.status || "revisar");

    const label=status==="correta"
      ? "Boa resposta"
      : status==="parcial"
        ? "Entendível · com ajustes"
        : "Vale revisar";

    return `
      <section
        class="english-inline-ai-correction ${esc(status)}"
        data-english-ai-inline-v81176
      >
        <header>
        ...
        ` : `
          <div>
            <b>Correção necessária</b>
            <p>Nenhuma correção objetiva importante nesta resposta.</p>
          </div>
        `}

        ${item?.naturalVersion ? `
          <div class="natural">
            <b>Forma mais natural</b>
            <p>${esc(item.naturalVersion)}</p>
          </div>
        ` : ""}
  ...
  function renderConversationInline(items=[]) {
    ...
    items.forEach((item,index)=>{
      ...
      const idNumber=Number(
        id.match(/(\d+)$/)?.[1] || index+1
      );

      const zeroIndex=Math.max(0,idNumber-1);
      const block=conversationQuestionBlock(zeroIndex);
      ...
      block.insertAdjacentHTML(
        "beforeend",
        conversationInlineHtml(item,zeroIndex)
      );
  ...
  function renderReadingInline(analysis={}) {
    const target=readingInlineTarget();
    if(!target) return false;

    target.querySelector(
      "[data-english-reading-feedback-v81176]"
    )?.remove();

    const wrapper=document.createElement("section");
    wrapper.className="english-inline-reading-feedback";
    wrapper.dataset.englishReadingFeedbackV81176="true";
    wrapper.setAttribute("data-english-ai-inline-v81176","");

    wrapper.innerHTML=`
      <div class="english-inline-reading-feedback__title">
        <span>CORREÇÃO DA LEITURA</span>
        <strong>Feedback da IA logo abaixo do texto</strong>
      </div>
  ...
  function renderInlineResult(state) {
    const analysis=state?.analysis || {};

    clearInlineFeedback();

    renderReadingInline(analysis);

    const inserted=renderConversationInline(
      analysis.conversation || []
    );

    if(
      Array.isArray(analysis.conversation)
      && inserted < analysis.conversation.length
    ){
      setTimeout(()=>{
        renderConversationInline(
          analysis.conversation || []
        );
      },450);
  ...
  function renderResult(state) {
    ...
    renderInlineResult(state);

    host.innerHTML=`
      <section class="english-final-clean english-final-clean--distributed">
        <header>
          <div>
            <span>CORREÇÃO CONCLUÍDA</span>
            <h2>Feedback colocado no lugar certo</h2>
            <p>
              A correção de cada resposta está logo abaixo da pergunta.
              A correção da Leitura está logo abaixo do texto.
            </p>
          </div>
        </header>
        ...
        ${analysis.focus ? `
          <div class="english-final-clean__summary focus">
            <span>UM PONTO PARA OBSERVAR NO PRÓXIMO ESTUDO</span>
  ...
  async function currentMaterial() {
    const weeklyReview=weeklyReviewMode();
    const [reading,remote,conversation]=await Promise.all([
      readingContext(),
      readConfig(FINAL_KEY),
      weeklyReview ? Promise.resolve([]) : collectConversation()
    ]);
    ...
    return {
      ...
      reading,
      weeklyReview,
      remote:remote?.date===ctx.data ? remote : null
    };
  }

  function renderStatus(material) {
    const conversationCount=material.conversation.length;
    const weeklyReview=Boolean(material.weeklyReview);
    ...
    const canFinish=
      (conversationCount>0 || weeklyReview)
      && readingOk
      && !waiting;
      ...
      <section class="english-final-clean">
        ...
        <div class="english-final-clean__checklist">
          <div class="${conversationMarkedComplete ? "ok" : ""}">
            <span>1 · ${weeklyReview ? "Revisão semanal" : "Conversa"}</span>
            <strong>${weeklyReview
              ? "erros reais da semana disponíveis para leitura"
              : conversationCount
  ...
  async function requestCorrection(canFinish,material) {
    ...
    if(!canFinish) {
      ...
      if(!material.conversation.length && !material.weeklyReview) {
        const stepText=norm(
    ...
    try {
      const request={
        ...
        weeklyReview:Boolean(material.weeklyReview),
