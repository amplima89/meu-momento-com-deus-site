    ...
    host.innerHTML = `
      <section class="english-final-correction">
        ...
        <div class="english-final-checklist">
          ...
          <div class="${dailyOk ? "ok" : ""}">
            <span>Leitura + produção</span>
            <strong>${dailyOk ? "salva e pronta" : "salve esta parte acima"}</strong>
          </div>
        ...
        ` : `
          <button
            type="button"
            class="btn primary english-final-button"
        ...
        <p class="english-final-helper">
          A conversa entra na correção quando houver respostas detectadas. Leitura/produção e Prática em contexto precisam estar concluídas.
        </p>
      </section>
    `;
  ...
  async function requestFinalCorrection() {
    ...
    try {
      ...
      if(!material.daily.ready) {
        window.MMCDUI?.toast?.("Salve primeiro a parte de leitura, writing e speaking.");
        return;
      }

      if(!material.practice.completed) {
        window.MMCDUI?.toast?.("Conclua primeiro a Prática em contexto.");
        return;
      }
