...
window.MMCDEnglishExperience = (() => {
  ...
  async function render({container,data,db,usuario}) {
    document.body.classList.add("english-v10","english-v807","english-v8113","english-v81135");
    ...
    if(window.MMCDConversationCorrectionV81135?.attach) {
      try {
        await window.MMCDConversationCorrectionV81135.attach({
          container,
          data,
          db,
          usuario
        ...
        console.warn("Inglês V81.13.5: falha ao conectar a correção detalhada da conversa.",error);
      }
    }
  }
