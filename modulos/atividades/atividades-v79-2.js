...
(async () => {
  ...
  function isMonthlyFlexible(meta) {
    return meta?.modoProgramacao === "mensal_flexivel";
  }
  ...
  function weeklyState(meta, date) {
    ...
    for (const day of dateRange(bounds.start, bounds.end)) {
      ...
      const row = MMCD.registro(d, day, meta.id);
      if (MMCD.estaAbonada(row)) excused += 1;
      else total += executionsOnDate(meta.id, day);
      const validTargetSoFar = Math.max(0, target - Math.min(target, excused));
      if (!completionDate && total >= validTargetSoFar && (total + excused) >= target) completionDate = day;
    }
    const validTarget = Math.max(0, target - Math.min(target, excused));
    const complete = total >= validTarget && (total + excused) >= target;
    const pct = validTarget === 0 ? 100 : Math.min(100, Math.round((total / validTarget) * 100));
    const ended = bounds.end < today;
    const partial = ended && total > 0 && !complete;
    const failed = ended && total === 0 && excused === 0;
    return {
      ...bounds,
      target,
      validTarget,
      excused,
      total,
      remaining: Math.max(0, validTarget - total),
      pct,
      ended,
      complete,
      partial,
      ...
      selectedExcused: MMCD.estaAbonada(MMCD.registro(d, date, meta.id))
    };
  }
  ...
  function monthlyTarget(meta) {
    return Math.max(1, Number(meta?.metaMensal || meta?.quantidade || 1));
  }

  function monthlyState(meta, date) {
    const bounds = monthBounds(date);
    const target = monthlyTarget(meta);
    let total = 0;
    let completionDate = "";
    for (const day of dateRange(bounds.start, bounds.end)) {
      if (!isMetaActiveByDate(meta, day)) continue;
    ...
    return { ...bounds, target, total, remaining: Math.max(0, target-total), pct, ended, complete, partial, failed, completionDate, selectedCount: executionsOnDate(meta.id,date) };
  }

  function monthlyFlexibleMetasFor(date) {
    return (d.metas || [])
      .filter(meta => isMonthlyFlexible(meta) && isMetaActiveByDate(meta, date))
      .filter(meta => {
        const state = monthlyState(meta,date);
        return !state.complete || !state.completionDate || date <= state.completionDate;
      });
  }

  function monthlyStatusText(state) {
    if (state.complete) return `${state.total} de ${state.target} · Meta mensal concluída`;
    if (state.partial) return `${state.total} de ${state.target} · ${state.pct}% · Mês encerrado parcialmente`;
    if (state.failed) return `0 de ${state.target} · Mês encerrado · Não concluída`;
    if (state.total > 0) return `${state.total} de ${state.target} · ${state.pct}% · faltam ${state.remaining} até o fim do mês`;
    return `0 de ${state.target} · Pendente · você tem até o fim do mês`;
  }

  function renderMonthlyGoal(meta) {
    const state=monthlyState(meta,selected);
    ...
    return `<article class="weekly-flex-goal monthly-flex-goal ${stateClass}">
      <div class="weekly-flex-goal__top"><span class="weekly-flex-goal__icon">${MMCDUI.esc(meta.icone||"◫")}</span><div class="weekly-flex-goal__copy"><div class="weekly-flex-goal__title"><strong>${MMCDUI.esc(meta.nome||"Meta mensal")}</strong><span>${state.pct}%</span></div><small>${MMCDUI.esc(monthlyStatusText(state))}</small></div></div>
      <div class="weekly-flex-progress"><i style="width:${state.pct}%"></i></div>
      <div class="weekly-flex-goal__footer"><span>${state.total}/${state.target} ${MMCDUI.esc(unit)} neste mês</span><div class="weekly-flex-actions">
        ${state.selectedCount>0?`<button class="btn small" type="button" data-monthly-minus="${meta.id}" ${canEdit?"":"disabled"}>− Desfazer hoje</button>`:""}
        ${!state.complete?`<button class="btn small primary" type="button" data-monthly-plus="${meta.id}" ${canEdit?"":"disabled"}>+ Registrar realização</button>`:`<span class="weekly-flex-done">✓ Concluída</span>`}
      </div></div></article>`;
  }

  async function changeMonthlyExecution(metaId,delta) {
    const meta=(d.metas||[]).find(item=>item.id===metaId);
    if(!meta||!isMonthlyFlexible(meta)) return;
    ...
    MMCD.setRegistro(d,selected,metaId,{concluida:current>0,abonada:false,valor:current,texto:"",observacao:previous?.observacao||"",origem:"mensal_flexivel"});
    render(); setStatus("Salvando meta mensal...","saving");
    try { d=await MMCD.salvarRegistroAtividade(d,selected,metaId); render(); const updated=monthlyState(meta,selected); setStatus(updated.complete?"Meta mensal concluída":"Progresso mensal salvo","success",true); }
    catch(error){ restoreRecord(metaId,previousState); render(); showError(error,"Não foi possível salvar a meta mensal."); }
  }

  function weeklyStatusText(state) {
    const abono = state.excused ? ` · ${state.excused} abono${state.excused===1?"":"s"}` : "";
    if (state.selectedExcused) return `Hoje abonado · não conta como falha${state.excused>1?` · ${state.excused} abonos na semana`:""}`;
    if (state.complete) return `${state.total} de ${state.validTarget} válidas · Meta semanal concluída${abono}`;
    if (state.partial) return `${state.total} de ${state.validTarget} válidas · ${state.pct}% · Semana encerrada parcialmente${abono}`;
    if (state.failed) return `0 de ${state.validTarget} válidas · Semana encerrada · Não concluída`;
    if (state.total > 0) return `${state.total} de ${state.validTarget} válidas · ${state.pct}% · falta${state.remaining === 1 ? "" : "m"} ${state.remaining} até domingo${abono}`;
    return `0 de ${state.validTarget} válidas · Pendente · você tem até domingo${abono}`;
  }
  ...
  function renderWeeklyGoal(meta) {
    ...
    const stateClass = excused ? "is-excused" : state.complete ? "is-complete" : state.failed ? "is-failed" : state.partial ? "is-partial" : "is-pending";
    return `<article class="weekly-flex-goal ${stateClass}">
      <div class="weekly-flex-goal__top">
        ...
        <div class="weekly-flex-goal__copy">
          ...
          ${reason ? `<small class="weekly-flex-excuse-reason">Motivo: ${MMCDUI.esc(reason)}</small>` : ""}
        </div>
      </div>
      <div class="weekly-flex-progress"><i style="width:${state.pct}%"></i></div>
      <div class="weekly-flex-goal__footer">
        <span>${state.total}/${state.validTarget} ${MMCDUI.esc(unit)} válidas nesta semana${state.excused?` · ${state.excused} abono${state.excused===1?"":"s"}`:""}</span>
        <div class="weekly-flex-actions">
          <button class="btn small weekly-flex-excuse ${excused?"active":""}" type="button" data-action="excuse" data-goal="${meta.id}" ${canEdit?"":"disabled"}>${excused?"✓ Abonado":"A · Abonar hoje"}</button>
          ${!excused && state.selectedCount > 0 ? `<button class="btn small" type="button" data-weekly-minus="${meta.id}" ${canEdit ? "" : "disabled"}>− Desfazer hoje</button>` : ""}
          ${!excused && !state.complete ? `<button class="btn small primary" type="button" data-weekly-plus="${meta.id}" ${canEdit ? "" : "disabled"}>+ Registrar realização</button>` : state.complete ? `<span class="weekly-flex-done">✓ Concluída</span>` : ""}
        </div>
      </div>
    </article>`;
  }
  ...
  async function changeWeeklyExecution(metaId, delta) {
    ...
    const state = weeklyState(meta, selected);
    let current = executionsOnDate(metaId, selected);
    if (delta > 0 && state.complete) return;
    current = Math.max(0, current + delta);
  ...
  async function toggleExcuse(button) {
    ...
    const goalId = button.dataset.goal;
    const goal = goalsFor(selected).find(meta => meta.id === goalId) || (d.metas || []).find(meta => meta.id === goalId);
    const previous = MMCD.registro(d, selected, goalId);
    const previousState = previous ? { ...previous } : null;
  ...
  function renderDay() {
    ...
    const monthlyFlexible = monthlyFlexibleMetasFor(selected);
    const monthlyHtml = monthlyFlexible.length ? `
      <section class="weekly-flex-section monthly-flex-section">
        <div class="weekly-flex-section__head"><div><span>MÊS FLEXÍVEL</span><strong>Até o último dia do mês</strong></div><small>Enquanto o mês estiver aberto, a meta continua pendente.</small></div>
        <div class="weekly-flex-list">${monthlyFlexible.map(renderMonthlyGoal).join("")}</div>
      </section>` : "";
    ...
    $("#daily-goals").innerHTML = monthlyHtml + weeklyHtml + regularHtml || '<div class="empty-day">Nenhuma meta programada para este dia.</div>';

    document.querySelectorAll('[data-monthly-plus]').forEach(button => { button.onclick = () => changeMonthlyExecution(button.dataset.monthlyPlus, 1); });
    document.querySelectorAll('[data-monthly-minus]').forEach(button => { button.onclick = () => changeMonthlyExecution(button.dataset.monthlyMinus, -1); });
