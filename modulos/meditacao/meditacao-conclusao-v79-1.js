...
(() => {
  ...
  function socialData(date) {
    ...
    const caption = [
      "Minha meditação de hoje no Memory 🙏",
    ...
    return {
      ...
      caption,
      shareTitle: "Minha meditação no Memory",
  ...
  function render() {
    ...
    zone.innerHTML = `
      <div class="meditation-completion-head">
        <div><p class="eyebrow">Feche este encontro</p><h2>${done ? "Meditação concluída" : "Concluiu sua meditação?"}</h2><p class="muted">${done ? "Este dia já está marcado nas suas Atividades." : "Ao concluir aqui, o Memory marca automaticamente a atividade de meditação desta data."}</p></div>
        <button type="button" class="btn ${done ? "meditation-complete-done" : "primary"}" data-meditation-complete ${busy ? "disabled" : ""}>${busy ? "Salvando…" : done ? "✓ Concluída" : "Concluir meditação"}</button>
      </div>
      ${done ? `
      <div class="memory-share-block">
        <div class="memory-share-preview memory-share-preview--meditation">
          <div class="memory-share-preview__brand"><img src="assets/imagens/memory-mark-v62.png" alt=""><div><strong>Memory</strong><small>um momento que vale lembrar</small></div></div>
          <span class="memory-share-preview__eyebrow">Meditação concluída</span>
  ...
  async function complete() {
    ...
    const meta = meditationMeta(date);
    if (!meta) {
      MMCDUI?.toast?.("Não encontrei uma atividade de Meditação para atualizar. Confira suas Metas/Atividades.", 5200);
      return;
    }
    ...
    try {
      ...
      MMCDUI?.toast?.("Meditação concluída e Atividades atualizadas.", 3800);
      document.dispatchEvent(new CustomEvent("memory:activity-updated", { detail: { date, activityId: meta.id, source: "meditation" } }));
    } catch (error) {
      console.error("Memory: não foi possível concluir a meditação.", error);
      MMCD.setRegistro(data, date, meta.id, previous);
      MMCDUI?.toast?.(error?.message || "Não foi possível atualizar a atividade de meditação.", 5200);
    } finally {
      busy = false;
      render();
    }
  ...
  async function refresh() {
    try {
      ...
      console.error("Memory: conclusão da meditação indisponível.", error);
    }
  }
