...
(() => {
  ...
  async function renderConsistency(){
    ...
    try{
      ...
      el.innerHTML=`<div class="consistency__top"><div><h2>CONSISTÊNCIA NA DEVOCIONAL</h2><p>Você está há <strong>${atual} ${atual===1?'dia':'dias'}</strong> consecutivos.</p><p>Seu recorde é <strong>${recorde} ${recorde===1?'dia':'dias'}</strong>.</p></div><strong>${validos30.length?Math.round(totalConcluido/validos30.length*100):0}%</strong></div><div class="consistency-grid">${ultimos30.map(date=>{const programada=meta&&MMCD.ativaNaData(meta,date);const estado=!programada?'off':concluida(date)?'on':'missed';const texto=!programada?'Não programada':concluida(date)?'Realizada':'Não realizada';return `<i class="consistency-cell ${estado}" title="${MMCDUI.date(date)} — ${texto}"></i>`}).join('')}</div>`;
