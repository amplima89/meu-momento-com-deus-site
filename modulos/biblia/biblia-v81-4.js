...
(() => {
  ...
  const CHAVE_IGREJA = "biblia_versiculos_igreja_v1";
  ...
  const statMostAdvancedBook = document.querySelector("#biblia-stat-most-advanced-book");
  const statLastReading = document.querySelector("#biblia-stat-last-reading");
  ...
  let marcacoesIgreja = { schemaVersion: 1, capitulos: {}, atualizadoEm: null };
  let salvandoIgreja = Promise.resolve();
  ...
  async function sincronizarLeituraComAtividades(data=dataLocalIso(new Date()),{silencioso=true}={}) {
    try {
      const resultado=await window.MemoryActivitySync?.mark?.("bible",{
        date:data,
        origin:"biblia_capitulo",
        observation:"Leitura registrada automaticamente a partir da Bíblia."
      });
      if(!silencioso && resultado?.ok && !resultado.already) {
        window.MMCDUI?.toast?.(`${resultado.meta?.nome || "Ler a Bíblia"} atualizada em Atividades.`);
      }
      return resultado;
    } catch(erro) {
      console.warn("Bíblia: não foi possível sincronizar a leitura com Atividades.",erro);
      return null;
    }
  }

  async function reconciliarLeiturasRecentesComAtividades() {
    const hoje=new Date();
    const limite=new Date(hoje);
    limite.setDate(limite.getDate()-60);
    const datas=new Set();

    Object.values(progresso?.capitulos || {}).forEach(item=>{
      if(!item?.concluido || !item?.concluidoEm) return;
      const quando=new Date(item.concluidoEm);
      if(Number.isNaN(quando.getTime()) || quando<limite || quando>hoje) return;
      const data=dataLocalIso(quando);
      if(data) datas.add(data);
    });

    Object.values(progresso?.livrosCompletos || {}).forEach(item=>{
    ...
    for(const data of [...datas].sort()) {
      await sincronizarLeituraComAtividades(data,{silencioso:true});
    }
  }
  ...
  function igrejaNormalizada(valor) {
    const v=valor&&typeof valor==="object"?valor:{};
    return {schemaVersion:1,capitulos:v.capitulos&&typeof v.capitulos==="object"?v.capitulos:{},atualizadoEm:v.atualizadoEm||null};
  }

  async function carregarMarcacoesIgreja(){
    const {data,error}=await db.from("configuracoes_usuario").select("valor").eq("user_id",usuario.id).eq("chave",CHAVE_IGREJA).maybeSingle();
    if(error)throw error;
    marcacoesIgreja=igrejaNormalizada(data?.valor);
  }

  function chaveIgreja(livroId=livroAtual().id,capitulo=capituloAtual()){return `${livroId}:${Number(capitulo)}`}
  function marcacoesIgrejaCapitulo(livroId=livroAtual().id,capitulo=capituloAtual()){return marcacoesIgreja.capitulos?.[chaveIgreja(livroId,capitulo)]||{}}
  function versiculoMarcadoNaIgreja(numero,livroId=livroAtual().id,capitulo=capituloAtual()){return Boolean(marcacoesIgrejaCapitulo(livroId,capitulo)?.[String(Number(numero))])}
  function totalMarcadoNaIgrejaAtual(){return Object.keys(marcacoesIgrejaCapitulo()).length}
  function salvarMarcacoesIgreja(){
    marcacoesIgreja.atualizadoEm=new Date().toISOString();
    const valor=JSON.parse(JSON.stringify(marcacoesIgreja));
    salvandoIgreja=salvandoIgreja.catch(()=>undefined).then(async()=>{const {error}=await db.from("configuracoes_usuario").upsert({user_id:usuario.id,chave:CHAVE_IGREJA,valor},{onConflict:"user_id,chave"});if(error)throw error});
    return salvandoIgreja;
  }
  async function alternarVersiculoIgreja(numero){
    const key=chaveIgreja();const verse=String(Number(numero));
    if(!marcacoesIgreja.capitulos[key])marcacoesIgreja.capitulos[key]={};
    const novo=!Boolean(marcacoesIgreja.capitulos[key][verse]);
    if(novo)marcacoesIgreja.capitulos[key][verse]={marcadoEm:new Date().toISOString()};else delete marcacoesIgreja.capitulos[key][verse];
    if(!Object.keys(marcacoesIgreja.capitulos[key]).length)delete marcacoesIgreja.capitulos[key];
    renderizar();
    await salvarMarcacoesIgreja();
    window.MMCDUI?.toast(novo?`Versículo ${numero} marcado como visto na igreja.`:`Marcação da igreja removida do versículo ${numero}.`);
  }
  ...
  async function fetchBibliaJson(url,{tentativas=2}={}) {
    let ultimoErro=null;
    ...
    for(let tentativa=0;tentativa<=tentativas;tentativa+=1) {
      try {
        ...
        const erro=new Error(`Resposta ${resposta.status}`);
        erro.status=resposta.status;
        ultimoErro=erro;

        // 404 não é transitório: cai imediatamente no próximo endpoint.
        if(resposta.status===404) break;

        // 429 e erros de servidor podem ser temporários.
        if(!(resposta.status===429 || resposta.status>=500)) break;
      } catch(erro) {
        ultimoErro=erro;
      }

      if(tentativa<tentativas) await esperar(650*(tentativa+1));
    }

    throw ultimoErro || new Error("Falha ao consultar a Bíblia online.");
  }

  function normalizarVersosApi(dados) {
    const lista=Array.isArray(dados?.verses) ? dados.verses : [];
    return lista
      .map(v=>({
        numero:Number(v?.verse),
        texto:String(v?.text||"").replace(/\s+/g," ").trim()
      }))
      .filter(v=>Number.isFinite(v.numero) && v.numero>0 && v.texto);
  }

  async function buscarVersosOnline(livro,capitulo) {
    const numero=Number(capitulo);

    // Endpoint principal: não depende de interpretar o nome do livro.
    // Usa diretamente tradução + BOOK_ID + capítulo.
    const urlPrecisa=`https://bible-api.com/data/${encodeURIComponent(TRADUCAO)}/${encodeURIComponent(livro.id)}/${numero}`;

    try {
      const dados=await fetchBibliaJson(urlPrecisa,{tentativas:2});
      const versos=normalizarVersosApi(dados);
      if(versos.length) return versos;
      throw new Error("Capítulo vazio no endpoint parametrizado.");
    } catch(erroPreciso) {
      console.warn("Bíblia: endpoint parametrizado falhou; tentando endpoint legado.",{
        livro:livro.id,
        capitulo:numero,
        erro:erroPreciso
      });

      // Fallback legado preservado por compatibilidade.
      // Aqui usamos o nome em português, pois a tradução ativa é Almeida.
      const referencia=`${livro.nome} ${numero}`;
      const urlLegada=`https://bible-api.com/${encodeURIComponent(referencia)}?translation=${encodeURIComponent(TRADUCAO)}&single_chapter_book_matching=indifferent`;
      const dados=await fetchBibliaJson(urlLegada,{tentativas:1});
      const versos=normalizarVersosApi(dados);

      if(!versos.length) throw erroPreciso;
      return versos;
    }
  }
  ...
  async function quantidadeVersosCapitulo(livro,capitulo) {
    ...
    const encontrados=await buscarVersosOnline(livro,capitulo);
    if(!encontrados.length)throw new Error("Capítulo sem versículos.");
  ...
  async function alternarCapitulo(livro,capitulo,opcoes={}) {
    ...
    if(novoEstado) await sincronizarLeituraComAtividades(dataLocalIso(new Date()),{silencioso:true});
  ...
  async function marcarLivroCompleto(livro) {
    ...
    Object.keys(progresso.capitulos).forEach(chave=>{if(chave.startsWith(`${livro.id}:`))delete progresso.capitulos[chave];});
    await salvarProgresso();
    await sincronizarLeituraComAtividades(dataLocalIso(new Date()),{silencioso:true});
    renderizarProgresso();
    window.MMCDUI?.toast(`${livro.nome} marcado como concluído.`);
  }
  ...
  function ultimaLeituraConcluida(){
    const items=[];
    Object.entries(progresso.capitulos||{}).forEach(([key,value])=>{if(!value?.concluido||!value?.concluidoEm)return;const [bookId,chapter]=key.split(":");const livro=LIVROS.find(x=>x.id===bookId);if(livro)items.push({livro,capitulo:Number(chapter),data:value.concluidoEm})});
    Object.entries(progresso.livrosCompletos||{}).forEach(([bookId,value])=>{if(!value?.concluidoEm)return;const livro=LIVROS.find(x=>x.id===bookId);if(livro)items.push({livro,capitulo:livro.capitulos,data:value.concluidoEm,livroCompleto:true})});
    return items.sort((a,b)=>new Date(b.data)-new Date(a.data))[0]||null;
  }
  function livroAtualDaJornada(){
    const ultima=ultimaLeituraConcluida();
    if(ultima){const item=dadosLivro(ultima.livro);if(!item.concluido)return item}
    const selecionado=dadosLivro(livroAtual());
    if(selecionado.capitulosConcluidos>0&&!selecionado.concluido)return selecionado;
    return melhorLivroEmAndamento();
  }
  function tempoLeitura(value){
    if(!value)return "—";const d=new Date(value);if(Number.isNaN(d.getTime()))return "—";const now=new Date();const today=new Date(now.getFullYear(),now.getMonth(),now.getDate());const day=new Date(d.getFullYear(),d.getMonth(),d.getDate());const diff=Math.round((today-day)/86400000);if(diff===0)return "hoje";if(diff===1)return "ontem";return day.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"});
  }

  function renderizarCabecalhoProgresso(){
    const resumo=resumoProgresso();const emAndamento=melhorLivroEmAndamento();const atualJornada=livroAtualDaJornada();const ultima=ultimaLeituraConcluida();
    ...
    if(progressMessage){
      ...
      else if(atualJornada){const faltam=atualJornada.livro.capitulos-atualJornada.capitulosConcluidos;progressMessage.textContent=`Agora você está em ${atualJornada.livro.nome}: ${atualJornada.capitulosConcluidos} de ${atualJornada.livro.capitulos} capítulos (${atualJornada.percentual}%). Faltam ${faltam} capítulo${faltam===1?"":"s"}.`;}
    ...
    if(statCurrentBook)statCurrentBook.textContent=atualJornada?`${atualJornada.livro.nome} · ${atualJornada.capitulosConcluidos}/${atualJornada.livro.capitulos} · ${atualJornada.percentual}%`:"—";
    if(statMostAdvancedBook)statMostAdvancedBook.textContent=emAndamento?`${emAndamento.livro.nome} · ${emAndamento.percentual}%`:"—";
    if(statLastReading)statLastReading.textContent=ultima?`${ultima.livro.nome} ${ultima.capitulo} · ${tempoLeitura(ultima.data)}`:"—";
  ...
  async function buscarCapitulo() {
    ...
    try {
      const resultado=await buscarVersosOnline(livro,capitulo);
      if(!resultado.length) throw new Error("Capítulo vazio");
  ...
  function renderizar() {
    ...
    conteudo.innerHTML = versos.map((verso, i) => {
      const igreja=versiculoMarcadoNaIgreja(verso.numero);
      return `<p class="bible-verse ${igreja?"is-church-marked":""}" data-verso="${verso.numero}" data-start="${mapas[i].inicio}" data-end="${mapas[i].fim}">
        <button type="button" class="bible-verse-church-check ${igreja?"done":""}" data-church-verse="${verso.numero}" aria-pressed="${igreja?"true":"false"}" title="${igreja?"Remover marcação da igreja":"Marcar versículo lido/citado na igreja"}">${igreja?"✓":"○"}</button>
    ...
    const igrejaCount=totalMarcadoNaIgrejaAtual();
    status.textContent = `${versos.length} versículos · ${igrejaCount?`${igrejaCount} marcado${igrejaCount===1?"":"s"} na igreja · `:""}marcações sincronizadas`;
  ...
  async function carregar() {
    ...
    // Atualiza imediatamente para nunca manter o título do livro anterior
    // quando a consulta online falhar.
    titulo.textContent=`${livro.nome} ${capitulo}`;
    ...
    try {
      const [capituloCarregado]=await Promise.all([
        buscarCapitulo(),
        carregarDestaques(),
        carregarAnotacoes()
      ]);
      ...
      versos=capituloCarregado;
      renderizar();
    } catch(erro) {
      if(meuToken!==tokenCarga)return;

      console.error("Falha ao carregar capítulo da Bíblia",{
        livro:livro.id,
        capitulo,
        erro
      });
      ...
      versos=[];
      titulo.textContent=`${livro.nome} ${capitulo}`;
      conteudo.innerHTML=`<div class="bible-error">
        <strong>Não foi possível carregar ${esc(livro.nome)} ${capitulo}.</strong>
        <span>A conexão com a Bíblia online falhou. Você pode tentar novamente sem sair desta página.</span>
        <button type="button" class="btn" data-biblia-retry>Tentar novamente</button>
      </div>`;
      status.textContent="Falha temporária ao carregar";
    }
  }
  ...
  conteudo.addEventListener("click", evento => {
    const retry=evento.target.closest("[data-biblia-retry]");
    if(retry){evento.preventDefault();carregar();return;}

    const churchVerse=evento.target.closest("[data-church-verse]");
    if(churchVerse){evento.preventDefault();evento.stopPropagation();alternarVersiculoIgreja(Number(churchVerse.dataset.churchVerse)).catch(erro=>{console.error(erro);window.MMCDUI?.toast("Não foi possível salvar esta marcação da igreja.")});return;}
  ...
  (async () => {
    try {
      ...
      preencherLivros();
      const [pref] = await Promise.all([carregarPreferencia(), carregarProgresso(), carregarMarcacoesIgreja()]);
      await reconciliarLeiturasRecentesComAtividades();
