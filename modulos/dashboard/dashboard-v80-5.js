...
(async () => {
  ...
  const [
    ...
    readUserConfig("memory_aniversarios_registros_v1")
  ]);
  ...
  // V79.9: Registro rápido foi movido para Cuidado > Registro rápido.
  // Os dados continuam carregados aqui apenas para sequência e estatísticas.
  document.querySelector("#quick-journal-card")?.remove();
  ...
  function hasAnyMemoryRecord(date) {
    const activityRows = d.registros?.[date];
    const hasActivityRecord = !!(activityRows && typeof activityRows === "object"
      && Object.values(activityRows).some(row => row && typeof row === "object" && Object.keys(row).length));

    const hasQuickJournal = diarioRapido.registros.some(item => item?.data === date && String(item?.texto || "").trim());
    const hasEnglish = englishStudyDates.has(date);
    const hasWorkout = workoutSessions.some(session => session?.data === date && ["concluido", "parcial"].includes(session?.status));
    const hasBirthdayCare = Object.values(birthdayRecordsValue?.dias?.[date] || {}).some(row => row?.concluida || row?.abonada);
    const hasBookCompletion = (d.livros?.concluidos || []).some(item => String(item?.dataConclusao || "").slice(0, 10) === date);

    return hasActivityRecord || hasQuickJournal || hasEnglish || hasWorkout || hasBirthdayCare || hasBookCompletion;
  }
  ...
  function streak() {
    ...
    // A sequência não zera de madrugada só porque hoje ainda não teve registro.
    // Se ainda não houve registro hoje, mede a sequência encerrada ontem.
    if (!hasAnyMemoryRecord(isoDate(cursor))) cursor.setDate(cursor.getDate() - 1);

    while (count < 3650 && hasAnyMemoryRecord(isoDate(cursor))) {
      count += 1;
  ...
  const birthdaysToday = (Array.isArray(birthdaysValue?.itens) ? birthdaysValue.itens : [])
    .filter(item => item?.ativo !== false
      && Number(item?.dia) === today.getDate()
      && Number(item?.mes) === today.getMonth() + 1
      && String(item?.nome || "").trim())
    .sort((a, b) => String(a.nome).localeCompare(String(b.nome), "pt-BR", { sensitivity: "base" }));

  const birthdayNames = birthdaysToday.map(item => String(item.nome).trim());
  const birthdayRecordsToday = birthdayRecordsValue?.dias?.[iso] && typeof birthdayRecordsValue.dias[iso] === "object"
    ? birthdayRecordsValue.dias[iso]
    : {};
  const birthdayExcusedToday = birthdaysToday.filter(item => birthdayRecordsToday[item.id]?.abonada).length;
  const birthdayDoneToday = birthdaysToday.filter(item => !birthdayRecordsToday[item.id]?.abonada && birthdayRecordsToday[item.id]?.concluida).length;
  const validTodayWithBirthdays = validToday + Math.max(0, birthdaysToday.length - birthdayExcusedToday);
  const doneWithBirthdays = done + birthdayDoneToday;
  const excusedTodayWithBirthdays = excusedToday + birthdayExcusedToday;

  const birthdayCard = birthdayNames.length
    ? [
        "🎂",
        birthdayNames.length === 1 ? "Aniversário de hoje" : "Aniversários de hoje",
        birthdayNames.join(" · "),
        birthdayNames.length === 1
          ? `Dê feliz aniversário para ${birthdayNames[0]}.`
          : `${birthdayNames.length} pessoas importantes fazem aniversário hoje.`
      ]
    : null;
  ...
  const cards = [
    ["🔥", "Sequência de cuidado", `${streak()} dias`, hasAnyMemoryRecord(iso) ? "Dias consecutivos com algum registro · inclui hoje" : "Dias consecutivos com algum registro · hoje ainda pode continuar"],
