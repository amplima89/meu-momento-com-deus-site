...
(() => {
  ...
  function catalogGuide(item) {
    return window.MMCD_TREINO_GUIAS?.[item?.guiaId || item?.id] || null;
  }

  function catalogCoachSummary(item) {
    const guide=catalogGuide(item);
    const coach=coachFor(item?.guiaId || item?.id,guide);
    ...
    return {
      position:guide?.passos?.[0] || "Ajuste o equipamento e monte uma posição estável.",
      movement:guide?.passos?.[1] || "Execute o movimento com controle.",
      avoid:guide?.dica || "Não sacrifique a técnica para aumentar a carga."
    };
  }
  ...
  function catalogCardHtml(item) {
    ...
    return `<article class="exercise-catalog-card"
      data-catalog-card
      data-catalog-search="${esc(normalizeText(`${item.nome} ${item.grupo} ${item.equipamento}`))}"
      data-catalog-group="${esc(item.grupo)}"
      data-catalog-equipment="${esc(item.equipamento)}">
      <div class="exercise-catalog-card__visual">
        <figure>
          <span>INÍCIO</span>
          <img src="${esc(guide.inicio)}" alt="${esc(item.nome)} — posição inicial" loading="lazy" referrerpolicy="no-referrer">
        </figure>
        <figure>
          <span>${guide.estatico?"MANTER":"FIM"}</span>
          <img src="${esc(guide.fim)}" alt="${esc(item.nome)} — posição final" loading="lazy" referrerpolicy="no-referrer">
        </figure>
      </div>
      ...
      <div class="exercise-catalog-card__body">
        <div class="exercise-catalog-card__title">
          <div>
            ...
            <small>${esc(item.grupo)} · ${esc(item.equipamento)}</small>
          </div>
        ...
        <div class="exercise-catalog-coach">
          ...
          <div><b>⚠ Não faça</b><p>${esc(coach.avoid)}</p></div>
        </div>

        ${guide.dica?`<div class="exercise-catalog-tip"><b>Dica do personal</b><span>${esc(guide.dica)}</span></div>`:""}

        <div class="exercise-catalog-card__actions">
          <button type="button" class="btn small" data-action="show-exercise-guide" data-guide-id="${esc(item.guiaId || item.id)}">Ver orientação completa</button>
          ${replacing
            ? `<button type="button" class="btn primary small" data-action="catalog-use-exercise" data-catalog-id="${esc(item.id)}">
                Substituir ${esc(current?.nome || "exercício")}
              </button>`
            : ""}
        </div>
      </div>
    ...
    return `<article class="card settings-block exercise-catalog-section" id="exercise-catalog-section">
      <div class="section-head">
        <div>
          ...
          <p class="muted">Fotos de início e fim, orientação técnica e substituição direta de um exercício do seu plano.</p>
        </div>
        <span class="db-badge">${exerciseCatalog().length} opções</span>
      </div>
      ...
      <div class="exercise-catalog-filters">
        <label>
          <span>Buscar</span>
          <input id="exercise-catalog-search" type="search" placeholder="Ex.: peito, Smith, remada...">
        </label>
        <label>
          <span>Grupo</span>
          <select id="exercise-catalog-group">
            <option value="">Todos</option>
            ${catalogGroups().map(group=>`<option value="${esc(group)}">${esc(group)}</option>`).join("")}
          </select>
        </label>
        <label>
          ...
          <select id="exercise-catalog-equipment">
            <option value="">Todos</option>
            ${catalogEquipments().map(eq=>`<option value="${esc(eq)}">${esc(eq)}</option>`).join("")}
          </select>
        </label>
      </div>

      <div class="exercise-catalog-grid" id="exercise-catalog-grid">
        ${exerciseCatalog().map(catalogCardHtml).join("")}
      </div>
  ...
  function applyExerciseCatalogFilters() {
    const query=normalizeText($("#exercise-catalog-search")?.value || "");
    const group=$("#exercise-catalog-group")?.value || "";
    const equipment=$("#exercise-catalog-equipment")?.value || "";

    let visible=0;

    $$("[data-catalog-card]").forEach(card=>{
      const matchQuery=!query || String(card.dataset.catalogSearch||"").includes(query);
      const matchGroup=!group || card.dataset.catalogGroup===group;
      const matchEquipment=!equipment || card.dataset.catalogEquipment===equipment;
      const show=matchQuery && matchGroup && matchEquipment;
      card.hidden=!show;
      if(show) visible+=1;
    });
    ...
    const finishArea=session.status==="em_andamento"
      ? `<article class="card finish-workout-card ${p.pct===100?"ready":""}">
          <div>
            ...
            <p>${p.pct===100
              ?"Confira seus registros. Se marcou algo por engano, toque no ✓ do exercício para desfazer. Só depois finalize."
              :"O treino permanece aberto. Você pode marcar, desmarcar, editar cargas e voltar depois."}</p>
          </div>
          ${p.pct===100
  ...
  function workoutConstancyExcuseInfo(date=todayIso()){
    ...
    // Para constância, qualquer meta ativa claramente ligada ao treino pode preservar
    // a sequência quando estiver abonada. O abono não soma um treino; apenas evita a quebra.
    const candidates=activeActivityMetas(date).filter(meta=>{
  ...
  function workoutSocialHtml(session){
    ...
    return `<div class="workout-social-block memory-share-block">
      <div class="memory-share-preview memory-share-preview--workout">
        <div class="memory-share-preview__brand"><img src="assets/imagens/memory-mark-official-v81-11-3.png?v=20260822-v81-11-3" alt=""><div><strong>Memory</strong><small>movimento que vira memória</small></div></div>
