...
(async()=>{
 ...
 async function hydrateSignals(reason="load"){
  ...
  try{
   ...
   const journalByCategory=(category,windowItems=recentJournal30)=>windowItems.filter(item=>norm(item?.categoria)===norm(category));
   const journalFaith30=journalByCategory('Fé');
   const journalFamily30=journalByCategory('Família');
   const journalWork30=journalByCategory('Trabalho');
   const journalPersonal7=journalByCategory('Pessoal',recentJournal7);
   const journalHealth7=journalByCategory('Saúde',recentJournal7);
   const journalDevelopment30=journalByCategory('Desenvolvimento');
   ...
   const journalMindScore=scoreByCount(journalPersonal7.length,3);
   const journalFaithScore=scoreByCount(journalFaith30.length,3);
   const journalFamilyScore=scoreByCount(journalFamily30.length,3);
   const journalWorkScore=scoreByCount(journalWork30.length,3);
   const journalHealthScore=scoreByCount(journalHealth7.length,3);
   const journalDevelopmentScore=scoreByCount(journalDevelopment30.length,3);
   ...
   const relationshipScore=mix(circleScore,goodRelationshipScore,intercessionScore,birthdayScore,journalFamilyScore);
   ...
   const developmentScore=mix(development.score,englishScore,goodDevelopmentScore,journalWorkScore,journalDevelopmentScore);
   ...
   const spiritualScore=mix(spiritual.score,prayerSpiritualScore,testimonySpiritualScore,journalFaithScore);
   ...
   const bodyWorkoutScore=body.score==null
     ?(recentWorkouts?C.clamp(Math.round(recentWorkouts/4*100)):null)
     :mix(body.score,recentWorkouts?C.clamp(recentWorkouts*25):null);
   const bodyScore=mix(bodyWorkoutScore,journalHealthScore);

   const updates={
    spiritual:{
      score:spiritualScore,
      evidence:`${recentPrayers30.length} oração(ões) movimentadas · ${recentTestimonies30.length} testemunho(s) · ${journalFaith30.length} registro(s) rápido(s) de Fé${spiritual.expected?` · ${spiritual.done}/${spiritual.expected} práticas espirituais previstas`:''}.`
    },
    body:{
      score:bodyScore,
      evidence:bodyScore==null?'Treinos e cuidados físicos começam a formar este sinal quando há registros.':`${recentWorkouts} treino(s) recente(s) · ${journalHealth7.length} registro(s) rápido(s) de Saúde${body.expected?` · ${body.done}/${body.expected} cuidados físicos previstos`:''}.`
    },
    mind:{
      score:mix(mindAct.score,journalMindScore),
      evidence:(mindAct.score==null&&!journalPersonal7.length)?'Ainda não há registros suficientes. Isso não significa que sua mente esteja mal — apenas que o Memory tem pouca evidência.':`${journalPersonal7.length} registro(s) rápido(s) Pessoal nos últimos 7 dias${mindAct.expected?` · ${mindAct.done}/${mindAct.expected} atividades relacionadas ao cuidado mental`:''}.`
    },
    relationships:{
      score:relationshipScore,
      evidence:`${circleOk}/${circleItems.length||0} pessoa(s) em Relacionamentos · ${recentGood30.length} boa(s) ação(ões) · ${intercessions30.length} oração(ões) por pessoas · ${birthdayItems.length} aniversário(s) · ${journalFamily30.length} registro(s) rápido(s) de Família.`
    },
    development:{
      score:developmentScore,
      evidence:`${development.expected?`${development.done}/${development.expected} práticas de estudo previstas`:'Sem metas de estudo previstas'}${englishScore!=null?` · Inglês ${englishScore}%`:''}${goodDevelopment30.length?` · ${goodDevelopment30.length} boa(s) ação(ões) ligada(s) ao trabalho`:''} · ${journalWork30.length+journalDevelopment30.length} registro(s) rápido(s) de Trabalho/Desenvolvimento.`
    },
    memories:{
      score:memoryScore,
   ...
   if(recentJournal30.length){
     insights.push(['📝','Registro rápido também alimenta o mapa',`${recentJournal30.length} registro(s) recente(s) foram relacionados à dimensão correspondente pela categoria escolhida e também a Memórias.`]);
   }
   if(light.length){
   ...
   if(circleItems.length&&circleOk<circleItems.length){
     insights.push(['❤️','Há pessoas fora da frequência de cuidado',`${circleItems.length-circleOk} pessoa(s) estão fora da frequência que você mesmo definiu em Relacionamentos.`]);
   }
   if(!insights.length){
