...
(async()=>{
  ...
  function inferWorkoutTrigger(m){
    const saved=String(m?.associacaoTreinoTipo||'').trim();
    if(workoutTriggers[saved]) return saved;
    const text=`${m?.nome||''} ${m?.descricao||''}`.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
  ...
  function refreshWorkoutIntegration(){
    const enabled=$('#goal-workout-link').checked;
    $('#workout-trigger-wrap').hidden=!enabled;
  }
  ...
  function open(m){
    ...
    $('#goal-workout-trigger').value=inferWorkoutTrigger(m);
  ...
  function render(){
    ...
    $('#goals-list').innerHTML=list.map(m=>{
      ...
      return `<article class="goal-item ${m.ativa?'':'inactive'}">
        ...
        <div class="goal-report-cell goal-category-cell" data-label="Grupo">
          ...
          ${m.associadaTreinoFisico?`<span class="workout-link-pill">${workoutTriggers[inferWorkoutTrigger(m)]?.icon||'🏃'} ${MMCDUI.esc(workoutTriggers[inferWorkoutTrigger(m)]?.label||'Treino')}</span>`:''}
        </div>
    ...
    document.querySelectorAll('[data-dup]').forEach(b=>b.onclick=async()=>{
      ...
      d.metas.push({...x,id:crypto.randomUUID(),nome:x.nome+' (cópia)',associadaTreinoFisico:false,associacaoTreinoTipo:inferWorkoutTrigger(x)});
      await MMCD.salvar(d);
      render();
  ...
  $('#goal-form').onsubmit=async e=>{
    ...
    const obj={
      ...
      associacaoTreinoTipo:$('#goal-workout-link').checked ? $('#goal-workout-trigger').value : inferWorkoutTrigger(d.metas.find(x=>String(x.id)===String(id)))
    };
