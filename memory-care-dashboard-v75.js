...
(async()=>{
  ...
  if(birthdaysToday.length)attention.push({icon:"🎂",title:`Hoje é aniversário de ${birthdaysToday[0].nome}`,text:"Um gesto simples de presença pode ser a coisa mais importante do dia.",href:"circulo-cuidado.html"});
  ...
  if(overdue.length){const p=overdue[0];attention.push({icon:"❤️",title:`Talvez valha cuidar de ${p.nome} hoje`,text:p.since>1000?"Você ainda não registrou um cuidado com essa pessoa.":`Último cuidado registrado há ${p.since} dias.`,href:"circulo-cuidado.html"})}
  const todayMetas=window.MMCD.metasNaData(d,iso).filter(m=>m?.ativa&&!window.MMCD.registro(d,iso,m.id)?.concluida);
  const spiritual=todayMetas.find(m=>C.normalizeName(`${m.nome} ${m.categoria}`).match(/deus|biblia|oracao|meditacao|devocional/));
  if(spiritual)attention.push({icon:spiritual.icone||"🙏",title:spiritual.nome,text:"Esta prática espiritual está prevista para hoje e ainda não aparece como concluída.",href:"atividades.html"});
  if(attention.length<3&&todayMetas.length){const m=todayMetas.find(x=>x!==spiritual)||todayMetas[0];attention.push({icon:m.icone||"✓",title:m.nome,text:"Uma atividade prevista para hoje ainda está aberta.",href:"atividades.html"})}
  const activePrayers=(prayers.itens||[]).filter(x=>x?.status!=="respondida");
  if(attention.length<3&&activePrayers.length)attention.push({icon:"🙏",title:"Há pedidos que você decidiu não esquecer",text:`${activePrayers.length} pedido${activePrayers.length===1?"":"s"} ativo${activePrayers.length===1?"":"s"} em Minhas Orações.`,href:"oracoes.html"});
  if(attention.length<3)attention.push({icon:"❤️",title:"Olhe o todo, não só a tarefa",text:"O Mapa de Cuidado reúne seis áreas da vida e mostra sinais recentes sem transformar você em uma nota.",href:"mapa-cuidado.html"});
  const mission=$("#mission-card");if(mission){const section=document.createElement("section");section.className="memory-today-panel";section.innerHTML=`<div class="memory-today-panel__head"><div><p class="eyebrow">Cuidado integral</p><h2>O que merece sua atenção hoje?</h2></div><small>Até 3 sinais relevantes</small></div><div class="memory-today-grid">${attention.slice(0,3).map(item=>`<article class="memory-today-item"><span>${item.icon}</span><strong>${C.esc(item.title)}</strong><small>${C.esc(item.text)}</small><a class="memory-care-card__link" href="${item.href}">Cuidar disso →</a></article>`).join("")}</div>`;mission.insertAdjacentElement("afterend",section)}
  ...
  if(overdue.length>=2)insights.push({icon:"❤️",title:"Relacionamentos estão pedindo espaço",text:`${overdue.length} pessoas do Círculo de Cuidado estão fora da frequência que você mesmo definiu.`});
